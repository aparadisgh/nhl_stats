...
#        print(team_roster[index]['position']['code'])
...
#                saves.append(data['teams'][homeaway[games]]['players']['ID'+str(playerID)]['stats']) #[
#                print(data['teams'][homeaway[games]]['players']['ID'+str(playerID)]['stats'].get('goalieStats'))
...
#                    saves.append(0)
                    ...
                    if stat_check2 == None:
                        ...
                        print(data['teams'][homeaway[games]]['players']['ID'+str(playerID)]['stats']['goalieStats']['saves'])  
...
def update_bar_chart(players, stats_sel, cumul_toggle):
    ...
    for player in players:#for each player, assign colour then create all subplots based on chosen stats with that same colour
        ...
        for plot in range(len(stats_sel)): #for all selected stats, add trace to each subplot for each player
            ...
            fig.add_scatter(x=stats[player].index, y=stats[player][stats_sel[plot]+conc], mode="lines+markers",
                    marker=dict(size=15, color=colour),
                    name=players_dict[player]+' ('+player_pos[player]+')', row=plot+1, col=1, showlegend=legend_status)
